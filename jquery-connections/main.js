...
jQuery(function($) {
  $('article').each( function() {
    $(this).on("click", function(event) {
      ...
      var $currentClass = $(this).attr('class').split(/\s+/); 
       // ^^-- this gets the list of classes and splits them seperately   
      console.log($currentClass);
      $('article').addClass('notsame');
      $(this).removeClass('notsame');
      $(this).toggleClass('compare'); // add checkmark to compare
 
     // fade out ones that arent the same
     $('article.'+$currentClass).removeClass('notsame');
     $('article.'+$currentClass).addClass('active');
     $('article.'+$currentClass).connections(); // add lines
      
      if ( $("article.compare").length === 2 ) {
       $('.button-compare').show();
     }

     if ( $("article.compare").length === 3 ) {
       $("article.compare").removeClass('compare');
       $('.button-compare').hide();
    ...
    $('.button-compare').on("click", function(event) {
      event.preventDefault();
      $('.modal').addClass('active');
      $('.button-compare').addClass('inactive');
      $(this).parent().next().find('.compare').clone().appendTo(".modal");
    });

   $('.button-clear').click(function(event) {
      event.preventDefault();
     $('article').removeClass("active notsame compare");
     $('.modal').removeClass("active");
     $('.button-compare.inactive').removeClass("inactive");
     $('.button-compare').hide();
     $('article').connections('remove');
